...
<\body>
  ...
  We consider the problem of how to use high throughput sequencing to infer
  ...
  <\enumerate-numeric>
    <item>Infer what proportion of cancer cells have a mutation
  ...
  fact that not all mutations will be heterozygous diploid events due to
  ...
  and statistically <with|font-shape|italic|unidentifiable>. Later when we
  introduce multiple samples, we will try to mitigate the impact of this
  assumption by allowing the mutational genotypes to differ between samples.
  ...
  With our simplified decomposition of populations and assumptions we can now
  ...
  apart) so that we have an infinite mixture of DNA fragments. Now the
  probability of sampling a DNA fragment from a cell depends on two things:
  first how common that type of cell was, second how many copies of the locus
  the cell has. We can easily derive these from the quantities we have
  ...
  <\remark>
    ...
    To see why this is necessary consider the following thought experiment.
    What would happen if one population had an infinite number of copies of
    the locus? Then it would contribute an infinite number of DNA fragments,
    and we would only sample these.
  </remark>

  <\remark>
    We can derive a slightly different model if we assume cells are first
  ...
  that read <math|i> was sampled from population <math|e>. Then
  ...
  reason to prefer one over the other, we simply assign all scenarios equal
  weight.
  ...
  mutations and nodes clonal populations. On the right is a hypothetical set
  ...
  and purple which all appear at the same point in the phylogeny. Assuming
  ...
  <with|font-shape|italic|non-parametric> Bayesian prior. Informally this
  means that it is a prior which can adapt model complexity as more data is
  observed. More formally the DP is a distribution over distributions
  ...
  we then say data points <math|i> and <math|j> belong to the sampled
  cluster. Now if we sample <math|\<zeta\><rsub|n>> from a continuous
  distribution, then there is zero probability that two data points will ever
  share the same value. So we need to sample <math|\<zeta\><rsub|n>> from a
  discrete distribution if there is any chance for shared values. In the
  background this is what the mixture model is doing, it is creating the
  ...
  now samples its own cellular prevalence <math|\<phi\><rsub|n>>. However,
  the values must belong to the set <math|<around*|{|\<theta\><rsub|1>,\<ldots\>,\<theta\><rsub|K>|}>>
  ...
  mixture model. The reason that it is useful is that if we sample a
  distribution <math|G> from a DP it will be discrete. More explicitly, any
  distribution <math|G> sampled from a DP has the following form
  ...
  Before discussing how to fit the model we will take a brief digression to
  ...
  customer enters the restaurant and sits down at a table. The second
  customer then enters the restaurant makes a choice. The can either join the
  first customer with probability <math|<frac|1|1+\<alpha\>>> or start a new
  table with probability <math|<frac|\<alpha\>|1+\<alpha\>>>. As new
  customers enter they can choose to sit at an existing table with
  probability proportional to the number of customers already there, or they
  can start a new table with probability proportional to <math|\<alpha\>>.
  The distribution over seatings of customers is then given by the CRP pmf.
  ...
  useful, as it will allow us to develop a Gibbs sampler for the DP in the
  next section.

  <subsubsection|Full model inference>\ 

  There are two approaches to performing inferences. One is to make use of
  the <with|font-shape|italic|stick breaking representation> of the DP. Using
  this approach we can sample the distribution <math|G> directly. Once we
  have this distribution, then we can perform Gibbs updates just like the
  finite mixture model case. The second approach which we will follow is
  marginalise <math|G> and in which case we have a CRP.\ 
  ...
  <\remark>
    This update is fairly easy to understand. The only subtlety occurs when
    we are updating a data point that is in a singleton cluster (only member
    of the cluster). In this case <math|K> differs when removing the data
    point from the clustering.
  </remark>
  ...
  since there is a cluster parameter associated with each cluster and this is
  used to generate the data point.
  ...
  parameters <math|l> is a tuning parameter for the algorithm. In practice we
  ...
  <\remark>
    ...
    paper) and other possible ways to perform inference for DPs.\ 
  </remark>
  ...
  cellular prevalences when we sample new tables. This is extremely slow as
  we do not use any information about the data to generate the new
  <math|\<phi\>> values.
  ...
  collecting samples. At each iteration we will record the cluster parameters
  ...
  different clustering of the data. The question is then how to pick a best
  clustering?\ 
  ...
  <math|k> to <math|\<ell\>>, nothing has changed because we are also
  changing the cluster parameters from <math|\<theta\><rsub|k>> to
  ...
  how to do this can be found in [cite mpear]. To implement this procedure we
  compute the pair-wise similarity matrix of two data points. This is the
  proportion of MCMC samples in which the data points belong to the same
  ...
  prevalence. One approach is to look at the full MCMC trace of the cellular
  prevalence associated with a mutation. We can plot these values as a
  ...
  prevalence given the clustering we can use a slightly ad-hoc approach and
  ...
  of <math|\<phi\><rsub|k>>. Then we need to compute the following quantity.
  ...
  The integral in the denominator does not have a closed form. But it is a
  one dimensional integral so we can use numerical methods to compute it.
  ...
  impossible to identify. If we have another sample where the prevalence is
  ...
  The main change is that we use the Uniform prior over the <math|M>
  dimensional unit cube and we sample a vector of cellular prevalence
  <math|\<b-phi\><rsub|n>=<around*|(|\<phi\><rsub|n
  1>,\<ldots\>,\<phi\><rsub|n m>|)>> for each mutation. The inference
  procedure is largely the same. The only major difference is that we will
  ...
  One issue with the current model is that read counts often exhibit more
  ...
  The Beta-Binomial has two parameters <math|a> and <math|b> like a Beta
  ...
  In this module we saw how to construct a Bayesian probabilistic model for
  inferring clonal population structure from bulk data. We started with a
  ...
  probabilistic model and fit it. The most important concepts are:
